...
async function generateDailySummary(botId: string, userId: string, dateStr: string): Promise<string> {
  const { startMs, endMs } = dateToRange(dateStr)
  const conversations = getConversationsByDateRange(botId, userId, startMs, endMs)
  const imageCaptions = getImageCaptionsByDateRange(botId, userId, startMs, endMs)
...
registerJobExecutor('daily_summary', async (job) => {
  ...
  return generateDailySummary(job.bot_id, job.user_id, targetDate)
})
...
const skill: Skill = {
  ...
  async execute(toolName, args, context): Promise<ToolResult> {
    if (toolName === 'generate_summary') {
      ...
      const summary = await generateDailySummary(context.botId, context.userId, dateStr)
      return { content: summary, exclusive: true }
    }

    if (toolName === 'enable_daily_digest') {
      return enableDigest(args.time as string, context.botId, context.userId)
    }

    if (toolName === 'disable_daily_digest') {
      return disableDigest(context.botId, context.userId)
    }
...
function enableDigest(time: string, botId: string, userId: string): ToolResult {
  ...
  const existing = getCronJobs(botId, userId).find((j) => j.job_type === 'daily_summary')
...
function disableDigest(botId: string, userId: string): ToolResult {
  const existing = getCronJobs(botId, userId).find((j) => j.job_type === 'daily_summary')
  if (!existing) {
